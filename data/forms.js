...
//
import { getBasicKit, getUpsellProducts } from './registrationProducts';
...
let formTemplates = [
      ...
      },
      {
        ...
        fields: [
          ...
          },
          {
            ...
            label: 'Select Team Shirt Design',
            required: true,
            order: 2,
  ...
  },
  {
    ...
    multiPage: true,
    welcomeBanner: {
      imageUrl: '/images/player-welcome.jpg',
        ...
        fields: [
          ...
          },
          {
            ...
            prefillFields: [
              ...
              { sourceFieldId: 35, sourceFieldLabel: 'Manager Contact' },
              { sourceFieldId: 5, sourceFieldLabel: 'Home Ground' }
            ]
          },
          {
            id: 34,
            type: 'sub-team-selector',
            label: 'Select Age Group Team',
            required: true,
            order: 3.5,
            dependsOn: 8,
            sourceFormId: 1,
            sourceFieldId: 33,
            helpText: 'Select which age group team you are joining'
          },
          {
            id: 24,
            type: 'image-select-library',
            label: 'Team Shirt Design',
            required: false,
            order: 4,
          ...
          },
          {
            ...
            type: 'number',
            label: 'Shirt Number',
            placeholder: 'Enter your preferred shirt number',
            required: true,
            order: 6,
            min: 1,
            max: 99
...
let formSubmissions = [
  {
    ...
    data: {
      1: 'Winter Warriors',
      2: 'John Smith',
      3: 'warriors@example.com',
      4: 'Under 15',
      5: 'Central Park Cricket Ground',
      22: 'https://example.com/warriors-logo.png',
      23: 'Classic Red Stripe'
    },
    submittedAt: new Date('2025-11-10T10:30:00').toISOString(),
    status: 'completed',
  ...
  },
  {
    ...
    data: {
      1: 'Thunder Strikers',
      2: 'Sarah Johnson',
      3: 'sarah.j@example.com',
      4: 'Under 14',
      5: 'Riverside Cricket Ground',
      22: 'https://example.com/striker-logo.png',
      23: 'Classic Red Stripe',
      '23_primaryColor': 'Red',
      '23_secondaryColor': 'Black'
    },
    submittedAt: new Date('2025-11-12T14:30:00').toISOString(),
    status: 'completed',
  ...
  },
  {
    ...
    data: {
      1: 'Ocean Warriors',
      2: 'David Martinez',
      3: 'dmartinez@example.com',
      4: 'Under 16',
      5: 'Central Cricket Park',
      22: 'https://example.com/ocean-logo.png',
      23: 'Blue Lightning',
      '23_primaryColor': 'Blue',
      '23_secondaryColor': 'Yellow'
    },
    submittedAt: new Date('2025-11-13T10:00:00').toISOString(),
    status: 'completed',
  ...
  },
  {
    ...
    data: {
      1: 'Lightning Bolts',
      2: 'Michael Chen',
      3: 'lightning@example.com',
      4: 'Under 12',
      5: 'Westside Cricket Oval',
      22: 'https://example.com/lightning-logo.png',
      23: 'Green Thunder',
      '23_primaryColor': 'Green',
      '23_secondaryColor': 'Black'
    },
    submittedAt: new Date('2025-11-15T09:15:00').toISOString(),
    status: 'completed'
  },
  {
    id: 4,
    ...
    data: {
      ...
      3: 'storm@example.com',
      4: 'Senior',
      5: 'Metropolitan Cricket Stadium',
      22: 'https://example.com/storm-logo.png',
      23: 'Yellow Storm',
      '23_primaryColor': 'Yellow',
      '23_secondaryColor': 'Navy'
    },
    submittedAt: new Date('2025-11-16T11:45:00').toISOString(),
    status: 'completed',
    approvalStatus: 'pending'
  },
  {
...
// Form Submissions
export function submitForm(formId, data) {
  const submission = {
    ...
    approvalStatus: 'pending' // pending, reviewed, complete
  };
  formSubmissions.push(submission);
  return submission;
}

export function getFormSubmissions(formId = null) {
  if (formId) {
    return formSubmissions.filter(sub => sub.formId === formId);
  }
  return formSubmissions;
}

export function getFormSubmissionsByFormId(formId) {
  return formSubmissions.filter(sub => sub.formId === formId);
}
...
export function getSubmissionById(submissionId) {
  return formSubmissions.find(sub => sub.id === submissionId);
}

export function updateSubmissionStatus(submissionId, status) {
  const index = formSubmissions.findIndex(sub => sub.id === submissionId);
  if (index !== -1) {
    formSubmissions[index].status = status;
    return formSubmissions[index];
  }
  return null;
}

export function updateApprovalStatus(submissionId, approvalStatus) {
  const index = formSubmissions.findIndex(sub => sub.id === submissionId);
  if (index !== -1) {
    formSubmissions[index].approvalStatus = approvalStatus;
    return formSubmissions[index];
  }
  return null;
}
...
export function resetSubmissionCounters() {
  // This function clears all submissions
  // In a production environment, you might want to keep the data and just reset a counter
  formSubmissions = [];
  submissionIdCounter = 1;
  return true;
}

export function resetSubmissionCounterForForm(formId) {
  // Remove all submissions for a specific form
  formSubmissions = formSubmissions.filter(sub => sub.formId !== formId);
  return true;
}
...
export function getFormWithProducts(formId) {
  ...
  if (formWithProducts.pages) {
    formWithProducts.pages = formWithProducts.pages.map(page => {
      if (page.pageId === 2) {
        // Update product fields with current data
      ...
      if (page.pageId === 3) {
        // Update supporter apparel products
        page.fields = page.fields.map(field => {
