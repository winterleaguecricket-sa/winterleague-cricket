...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (view === 'stats') {
      try {
        ...
        const tierDist = await query(`
          SELECT performance_tier, COUNT(*)::int AS count
          FROM suppliers WHERE status = 'approved' AND active = true
          GROUP BY performance_tier
        `);
        const tiers = { bronze: 0, silver: 0, gold: 0, platinum: 0 };
        tierDist.rows.forEach(r => { tiers[r.performance_tier] = r.count; });
    ...
    // Performance overview — tier history + rankings
    if (view === 'performance-overview') {
      try {
        const [rankings, recentChanges] = await Promise.all([
          query(`
            SELECT s.id, s.company_name, s.performance_tier, s.rating, s.total_sales,
                   s.total_revenue, s.sla_compliance_rate, s.featured,
                   COALESCE(AVG(p.quality_rating), 0)::DECIMAL(3,2) AS avg_product_quality
            FROM suppliers s
            LEFT JOIN products p ON p.supplier_id = s.id AND p.approval_status = 'approved' AND p.quality_rating IS NOT NULL
            WHERE s.status = 'approved' AND s.active = true
            GROUP BY s.id
            ORDER BY s.performance_tier DESC, s.rating DESC
          `),
          query(`
            SELECT h.*, s.company_name
            FROM supplier_performance_history h
        ...
        return res.status(200).json({
          rankings: rankings.rows.map(r => ({
            id: r.id, companyName: r.company_name, performanceTier: r.performance_tier,
            rating: parseFloat(r.rating || 0), totalSales: parseInt(r.total_sales || 0),
            totalRevenue: parseFloat(r.total_revenue || 0),
            slaComplianceRate: parseFloat(r.sla_compliance_rate || 100),
            avgProductQuality: parseFloat(r.avg_product_quality || 0),
            featured: r.featured
          })),
          recentChanges: recentChanges.rows.map(h => ({
            id: h.id, supplierId: h.supplier_id, companyName: h.company_name,
            oldTier: h.old_tier, newTier: h.new_tier, score: parseFloat(h.score || 0),
            rating: parseFloat(h.rating || 0), slaCompliance: parseFloat(h.sla_compliance || 0),
            totalSales: parseInt(h.total_sales || 0), reason: h.reason,
            createdAt: h.created_at
          }))
        });
      } catch (error) {
        console.error('Performance overview error:', error);
        return res.status(500).json({ error: 'Failed to fetch performance overview' });
      }
    }
  ...
  if (req.method === 'POST') {
    ...
    // Override performance tier (admin manual)
    if (action === 'override-performance-tier') {
      const { supplierId, performanceTier } = req.body;
      if (!supplierId || !performanceTier) return res.status(400).json({ error: 'Supplier ID and performance tier required' });
      const validTiers = ['bronze', 'silver', 'gold', 'platinum'];
      if (!validTiers.includes(performanceTier)) return res.status(400).json({ error: 'Invalid performance tier' });
      try {
        const current = await query('SELECT performance_tier, company_name FROM suppliers WHERE id = $1', [supplierId]);
        if (current.rows.length === 0) return res.status(404).json({ error: 'Supplier not found' });
        const oldTier = current.rows[0].performance_tier || 'bronze';
        await query('UPDATE suppliers SET performance_tier = $1, updated_at = NOW() WHERE id = $2', [performanceTier, supplierId]);
        // Record in history
        await query(`
          INSERT INTO supplier_performance_history 
            (supplier_id, old_tier, new_tier, score, reason)
          VALUES ($1, $2, $3, 0, 'admin_override')
          ON CONFLICT DO NOTHING
        `, [supplierId, oldTier, performanceTier]);
        return res.status(200).json({ success: true, message: `Performance tier updated to ${performanceTier}` });
      } catch (error) {
        console.error('Override performance tier error:', error);
        return res.status(500).json({ error: 'Failed to override performance tier' });
      }
    }
